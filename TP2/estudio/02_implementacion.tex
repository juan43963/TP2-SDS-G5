...
\portada{2}{Implementación}{%
  ...
  Arquitectura del motor y paso temporal}
...
\ojo{\textbf{Cambio respecto de la versión anterior del deck.} La diapositiva 7
\emph{era} ``Búsqueda de vecinos: Cell Index Method'' y se reemplazó por
``Paso temporal: actualización sincrónica''. \textbf{El Cell Index Method ya no
tiene diapositiva propia}: en el guion sólo se lo nombra al pasar, dentro de la
diapositiva 6, como el paso 1 del diagrama. Todo el material del CIM sigue
completo en este cuadernillo ---en \emph{El fondo} y en las preguntas--- porque
es lo más probable que te pregunten, y vuelve a aparecer en pantalla recién en
la diapositiva 25 (benchmark del inciso g, expositor C). No improvises una
explicación larga del CIM sobre la diapositiva 6: no hay tiempo y no es lo que
muestra la pantalla.}
...
A la izquierda el diagrama de flujo del paso temporal; a la derecha tres
puntos. Recorré el diagrama de arriba hacia abajo y cerrá con lo único que
distingue a los dos modelos.
...
Index Method ---el del TP1--- para obtener la lista de vecinos; después calcula
las orientaciones nuevas con la regla de alineación; y por último integra las
posiciones aplicando las condiciones periódicas. Los dos modelos comparten
absolutamente todo el código salvo esa función de alineación.''
\end{quote}

\clave{El punto que no podés dejar de decir en esta diapositiva: \textbf{los dos
modelos comparten todo el motor menos una función}. Es lo que hace legítima la
comparación de la sección de resultados ---lo que se mide es la diferencia entre
las reglas y no diferencias de implementación--- y es la frase que retoma C en
la comparación.}

\ojo{Nombrá el Cell Index Method y seguí. No lo expliques acá: no tiene
diapositiva y te comés el tiempo de la 7. Si querés dejar la puerta abierta,
alcanza con ``reutilizamos el Cell Index Method del TP1''; si les interesa, te
lo van a preguntar y tenés la respuesta completa más abajo.}

\diapo{7}{Paso temporal: actualización sincrónica}{45 s}{B}

Tres cajas en fila ---Vecinos, Ángulos, Mover--- y dos puntos debajo. Recorré
las cajas de izquierda a derecha y cerrá con el porqué.

\begin{quote}
\itshape
``Vale la pena detenerse en cómo está ordenado el paso, porque es lo que hace
que esto sea un autómata celular bien definido. Primero se reconstruye la grilla
de vecinos sobre las posiciones del instante $t$. Segundo, se calculan todas las
orientaciones nuevas, y acá está el punto: cada partícula lee \emph{únicamente}
el estado viejo, y el resultado se deposita en un buffer aparte, no se escribe
sobre la partícula. Tercero, se avanzan las posiciones ---con la velocidad
vieja, la que viene del ángulo de $t$--- y recién ahí se confirman los ángulos
nuevos. Ese doble buffer es lo que hace que la actualización sea sincrónica:
ninguna partícula ve el ángulo actualizado de otra dentro del mismo paso. Si no
lo hiciéramos así, la partícula de índice alto vería ángulos nuevos de las de
índice bajo, el resultado dependería del orden en que recorremos el vector
---que es arbitrario--- y dejaría de ser el modelo.''
\end{quote}

\clave{La idea, en una frase: \emph{``las orientaciones nuevas van a un buffer
aparte, así que todas las partículas deciden mirando la misma foto del
sistema''}. Y el corolario: sin eso, el resultado dependería del orden de
recorrido.}

\dato{Hay un segundo detalle en esa misma diapositiva que conviene decir aunque
sea al pasar, porque es una decisión y no un descuido: \textbf{la posición se
avanza con la velocidad vieja}, $\vect{x}(t+\Delta t) = \vect{x}(t) +
\vect{v}(t)\,\Delta t$, tal como la escribe la diapositiva 42 de la Teórica 2.
La partícula primero avanza en la dirección que traía y después gira. Está
desarrollado abajo y es una pregunta muy probable.}
...
Las cinco primeras son del Cell Index Method. \textbf{Ninguna está en pantalla}
desde que la diapositiva 7 pasó a ser el paso temporal: son material de
respuesta, no de guion. Las que sí sostienen lo que se ve en la pantalla son
\emph{La actualización sincrónica, en tres pasadas} y \emph{La convención de la
posición}; si vas justo de tiempo para estudiar, empezá por esas dos y por
\emph{El vecindario es autoinclusivo}.
...
\pregunta{Hablan del vecindario de $3\times3$, pero ¿el código recorre las 9
celdas?}
