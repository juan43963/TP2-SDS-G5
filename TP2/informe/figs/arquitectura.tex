...
\begin{tikzpicture}[
  ...
  % ---------------- Fase 1: CIM ----------------
  \node[box=4.9cm, fill=white, below=1.0cm of estado] (rebuild)
  ...
  % Resincronizacion del grid para los observables. Se rutea por la derecha de
  % TODO el diagrama (mas alla de la columna de observables) para no cruzar la
  % bifurcacion NeighborList -> {regla, observables}, que es donde la version
  % anterior de esta figura superponia la etiqueta con la caja NeighborList.
  \coordinate (syncx) at ($(obs.east)+(1.35,0)$);
  \draw[flecha2] (integra.east) -- (integra.east -| syncx)
    -- (rebuild.east -| syncx)
    -- node[etiqueta, above, midway] {\texttt{syncNeighbors()}}
       ($(rebuild.east)+(0,-0.32)$);
